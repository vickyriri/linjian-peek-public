...
- 新增按小时的使用分钟、亮屏分钟、未归因分钟和解锁次数。
- 从 Android UsageEvents 重建 App 使用区间，支持跨零点裁切、切换闭合和息屏闭合。
- 新增显式数据可信度：事件数、归因覆盖率、未归因时间、截断状态和说明。
- MCP 新增单一主入口 `get_checkin_snapshot`，一次返回四项核心数据与当前状态。
- 所有旧 MCP 工具继续注册和可调用；本次是兼容式软瘦身，不删除、不隐藏、不停用旧能力。
- Android 版本号提升为 `0.3.6.6` / `30606`，固定签名保持不变，可覆盖安装 Astra Custom v0.2。

详细字段和可信度口径见 [`docs/astra-custom-v0.3.md`](docs/astra-custom-v0.3.md)。
...
3. 构建成功后下载 `zhangxinchuang-public-v0.3.6.5-apk` artifact。

构建产物为：

```text
android/Zhangxinchuang-public-v0.3.6.5.apk
```

版本名 `0.3.6.5`，版本码 `30605`。
...
> v0.3.6.5 补充：无障碍状态会在从系统设置返回后延迟复查多次，并兼容不同系统写入无障碍组件名的格式差异；如果侧载 APK 被系统拦截，App 会提示去“应用信息 → 允许受限设置”，再回无障碍开启“掌心窗服务”。
